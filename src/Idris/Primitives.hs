...
primitives =
  ...
  [iCoerce (ITFixed IT8) (ITFixed IT16) "zext" zext LZExt,
   ...
   -- Buffers
   -- !!! TODO soon: add reductions for these using ByteStrings
   Prim (sUN "prim__allocate") (ty [AType (ATInt (ITFixed IT64))] BufferType) 1 (p_cantreduce)
    (1, LAllocate) total,
   Prim (sUN "prim__peekBits8") (ty [BufferType, AType (ATInt (ITFixed IT64))] . AType . ATInt $ ITFixed IT8) 2 (p_cantreduce)
    (2, LPeek) partial,
   Prim (sUN "prim__appendBits8") (ty [BufferType, AType (ATInt (ITFixed IT64)), AType (ATInt (ITFixed IT8))] BufferType) 3 (p_cantreduce)
    (3, LAppend) partial
